...
Goal
----
Lightweight static scan to reduce risk when using *third‑party* ("open") DashClaw skills.
...
def default_scan_paths() -> List[Path]:
    """Default to first-party local skills + installed dashclaw skills."""
    paths: List[Path] = []

    # Workspace first-party skills
    paths.append(Path.cwd() / "skills")

    # Installed dashclaw skills (global npm). Allow override.
    # NOTE: safe to scan read-only; this is a review tool.
    npm_skills = Path(os.environ.get(
        "DASHCLAW_INSTALLED_SKILLS_DIR",
        "",
    ))
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(description="Static safety scan for DashClaw skills")
    ...
    scan.add_argument(
        "--paths",
        nargs="*",
        help="Paths to scan (defaults: workspace skills + installed dashclaw skills)",
    )
    scan.add_argument(
        "--json",
        dest="json_out",
