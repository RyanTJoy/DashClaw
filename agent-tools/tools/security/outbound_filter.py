...
def main():
    ...
    scan_parser.add_argument('--push', action='store_true', help='Also push to DashClaw dashboard')
    ...
    if args.command == 'scan':
        ...
        if getattr(args, 'push', False):
            try:
                config = load_config()
                ok, res = push_to_api('/api/security/scan', {
                    'content': content,
                    'agent_id': config['agent_id'],
                })
                print(f"  Dashboard sync: {'OK' if ok else 'FAILED'} - {res if not ok else 'synced'}")
