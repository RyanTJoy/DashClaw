...
def main():
    parser = argparse.ArgumentParser(description='DashClaw Context Manager')
    ...
    capture_parser.add_argument('--push', action='store_true', help='Also push to DashClaw dashboard')
    ...
    thread_parser.add_argument('--push', action='store_true', help='Also push to DashClaw dashboard')
    ...
    if args.command == 'init':
        ...
        if getattr(args, 'push', False):
            try:
                config = load_config()
                ok, result = push_to_api('/api/context/points', {
        ...
        if getattr(args, 'push', False):
            try:
                config = load_config()
                ok, result = push_to_api('/api/context/threads', {
                    'name': args.name,
                    'agent_id': config['agent_id'],
                })
                print(f"  Dashboard sync: {'OK' if ok else 'FAILED'} - {result if not ok else 'synced'}")
