#!/usr/bin/env python3
"""
DashClaw Relationship Tracker
Track contacts, interactions, and follow-ups across platforms.

Usage:
    python tracker.py add <name> --platform twitter --handle @user
    python tracker.py list [--hot] [--due] [--platform twitter]
...
def main():
    parser = argparse.ArgumentParser(description="DashClaw Relationship Tracker")
    subparsers = parser.add_subparsers(dest='command', help='Commands')
    
    # Add contact
    add_parser = subparsers.add_parser('add', help='Add a new contact')
    add_parser.add_argument('name', help='Contact name')
    add_parser.add_argument('--platform', '-p', default='twitter', help='Platform (twitter, linkedin, email)')
    ...
    log_parser.add_argument('--push', action='store_true', help='Also push to DashClaw dashboard')
    ...
    if args.command == 'add':
        ...
        if getattr(args, 'push', False):
            try:
                config = load_config()
                ok, result = push_to_api('/api/relationships', {
                    'contact_name': getattr(args, 'name', '') or '',
                    'type': getattr(args, 'type', '') or '',
                    'direction': getattr(args, 'direction', '') or '',
                    'summary': getattr(args, 'summary', '') or '',
                    'agent_id': config['agent_id'],
                })
                print(f"  Dashboard sync: {'OK' if ok else 'FAILED'} - {result if not ok else 'synced'}")
