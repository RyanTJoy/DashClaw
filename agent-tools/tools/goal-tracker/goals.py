...
def main():
    ...
    add_parser.add_argument('--push', action='store_true', help='Also push to DashClaw dashboard')
    ...
    if args.command == 'init':
        ...
        if getattr(args, 'push', False):
            try:
                config = load_config()
                ok, result = push_to_api('/api/goals', {
                    'name': args.title,
                    'category': getattr(args, 'category', '') or '',
                    'target_date': getattr(args, 'target', '') or '',
