...
def main():
    if len(sys.argv) < 2:
        ...
        print("Commands:")
        print("  scan [--push]  - Run full health scan (--push syncs to DashClaw)")
    ...
    if cmd == 'scan':
        ...
        if '--push' in sys.argv:
            try:
                config = load_config()
                ok, api_result = push_to_api('/api/memory', {
                    'health': {
                        'score': result['health_score'],
                        'total_files': result['total_files'],
                        'total_lines': result['total_lines'],
                        'total_size_kb': result['total_size_kb'],
                        'memory_md_lines': result['memory_md_lines'],
                        'daily_files': result['daily_files'],
                        'duplicates': result['duplicates'],
                        'stale_count': result['stale_count'],
                    },
                    'agent_id': config['agent_id'],
                })
                print(f"  Dashboard sync: {'OK' if ok else 'FAILED'} - {api_result if not ok else 'synced'}")
