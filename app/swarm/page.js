...
export default function SwarmIntelligencePage() {
  ...
  const dragRef = useRef({ isDragging: false, startX: 0, startY: 0, node: null });
    ...
    const render = () => {
      ...
      nodes.forEach(node => {
        ...
        if (isSel || isHov) {
          ...
          const grad = ctx.createRadialGradient(node.x, node.y, 0, node.x, node.y, 25);
        ...
        // Body
        ctx.beginPath();
        ctx.arc(node.x, node.y, isSel ? 10 : 6, 0, Math.PI * 2);
        ctx.fillStyle = '#111';
  ...
  const handleMouseDown = (e) => {
    ...
    if (clickedNode) {
      dragRef.current = { isDragging: true, node: clickedNode };
      setSelectedAgentId(clickedNode.id);
      setNodeFixed(clickedNode.id, clickedNode.x, clickedNode.y);
    } else {
      dragRef.current = { isDragging: true, node: null, startX: e.clientX, startY: e.clientY };
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-6">
            ...
            <CardContent className="p-0 h-[600px] relative">
              ...
              >
                ...
                <div className="absolute top-4 right-4 flex flex-col gap-2">
                  <button onClick={() => setZoom(z => Math.min(5, z * 1.5))} className="w-8 h-8 rounded-lg bg-black/80 border border-white/10 text-white flex items-center justify-center hover:bg-brand/40 transition-colors">+</button>
        ...
        <div className="space-y-6">
          <Card className="h-full border-brand/5 bg-surface-secondary/20 shadow-xl backdrop-blur-lg">
