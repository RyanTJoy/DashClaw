/**
 * Billing utilities — plan limits, usage metering, and quota checking.
 *
 * Usage meters provide a fast path for quota enforcement:
...
// 10% grace buffer before hard block; 80% triggers warning header
const GRACE_BUFFER = 0.10;
...
/**
 * Returns the current UTC billing period as 'YYYY-MM'.
 */
 ...
 * Falls back to seedMeters() on cold start (first request of billing period).
 */
...
/**
 * Fast quota check — reads a single meter row instead of running COUNTs.
 * Falls back to seed on cold start.
 *
 * @param {string} orgId
 * @param {string} resource - One of: actions_per_month, agents, members, api_keys
 * @param {string} plan - Plan name (free/pro/team/enterprise)
 * @param {object} sql - Neon SQL driver
 ...
 * Delegates to checkQuotaFast for meter-based enforcement.
 *
 * @param {string} orgId
 * @param {string} resource - One of: actions_per_month, agents, members, api_keys
 * @param {string} plan - Plan name (free/pro/team/enterprise)
